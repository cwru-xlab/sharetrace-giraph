...
public abstract class ContractedPdaVentilator<T> implements Ventilator<T> {
  ...
  public void handleRequest() {
    ShortLivedTokenRequest tokenRequest = ShortLivedTokenRequest.builder()
        .longLivedToken(HandlerUtil.getEnvVar(LONG_LIVED_TOKEN, logger))
  ...
  private URL getContractsServerUrl() {
    ...
    try {
      contractsServerUrl = new URL(HandlerUtil.getEnvVar(CONTRACTS_SERVER_URL, logger));
  ...
  public List<String> getWorkers() {
    return HandlerUtil.getWorkers(workerKeys, logger);
  }
  ...
  protected String getContractId() {
    return HandlerUtil.getEnvVar(CONTRACT_ID, logger);
  }
