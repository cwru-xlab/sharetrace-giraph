 ...
 * functions. After this initial invoking, the other two Lambda functions will call each other until
 * all objects in S3 have been processed. The process can be expressed in pseudocode as:
 * invokeParentFunction(S3Event); // Invoked by S3 trigger parentFunction.invokeFirstChildFunction(S3Event);
 * // Invoked when remaining time is low firstChild.invokeSecondChild(S3Event); // Invoked when
 * remaining time is low secondChild.invokeFirstChild(S3Event); // Invoked when remaining time is
 * low ...
 */
public class ContactMatchingHandler implements RequestHandler<S3Event, String> {
  ...
  // TODO What if there is more than one record in the S3Event?
  @Override
