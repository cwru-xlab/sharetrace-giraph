...
def compute_msg(G, variable_node, factor_node, risks, curr_date):
    max_msg = 0
    curr_time = curr_date
    ...
    for i in range(len(G.nodes[factor_node]['cont_inf'])):
        ...
        for j in range(len(risks)):
            rsk_val = risks[j]
            if datetime.datetime.strptime(rsk_val[0],'%d/%m/%Y').date() <= datetime.datetime.strptime(t, '%d/%m/%Y').date() + datetime.timedelta(2):
                stm = True
                ind = j
                break
        if stm:
            ...
            tot_weight = 0
            msg = 0
            for j in range(ind, len(risks)):
                rsk_val = risks[j]
                msg = msg + np.exp(day_cnt) * rsk_val[1]
                tot_weight = tot_weight + np.exp(day_cnt)
                day_cnt = day_cnt - 1
            fin_msg = msg * trans_rate / tot_weight
            if fin_msg > max_msg:
                max_msg = fin_msg
                curr_time = t
            if fin_msg == max_msg and datetime.datetime.strptime(t,'%d/%m/%Y').date() > datetime.datetime.strptime(curr_time, '%d/%m/%Y').date():
                max_msg = fin_msg
                curr_time = t
...
def compute_message_factor_to_variable(G, factor_node, variable_node, neigh, curr_date):
    for i in range(len(neigh)):
        if neigh[i] != variable_node:
            ...
            #sort the risk scores according to time and then value
            sorted_risk = {k: v for k, v in sorted(comb_dict.items(), key=lambda x: (datetime.datetime.strptime(x[0],'%d/%m/%Y'), x[1]), reverse=True)}
            sorted_risks = list(sorted_risk.items())
            compute_msg(G, variable_node, factor_node, sorted_risks, curr_date)
